...
def run_ks(y_true, y_prob, ks_part=KS_PART):
    ...
    for i in np.arange(ks_part):
        if i == ks_part - 1 or data[length * (i + 1) / ks_part - 1, 1] != data[length * (i + 2) / ks_part - 1, 1]:
            ...
            if i != ks_part - 1:
                cut_pos = _get_cut_pos(data[length * (i + 1) / ks_part - 1, 1], data[:, 1],
                                       length * (i + 1) / ks_part - 1,
                                       length * (i + 2) / ks_part - 2)  # find the position of the rightest cut
            else:
                cut_pos = length - 1
    ...
    try:
        ...
        if len(cut_list) > 2:
            for i in range(2, len(cut_list)):
                span_list.append('(%.3f,%.3f]' % (round(cut_list[i - 1], 3), round(cut_list[i], 8)))
    except:
        span_list = ['0']
...
def print_ks(ks_info, ks_detail):
    ks_write = open(ks_detail, 'a+')
    ks_write.write('ks = %.1f%%\n' % ks_info['ks'])
    ks_write.write('\t'.join(['seq', '评分区间', '订单数', '逾期数', '正常用户数', '百分比', '逾期率', '累计坏账户占比', '累计好账户占比', 'KS统计量']))
    ks_write.write('\n')
    for i in range(len(ks_info['ks_list'])):
        ks_write.write('%d\t%s\t%d\t%d\t%d\t%.2f%%\t%.2f%%\t%.2f%%\t%.2f%%\t%.2f%%\n' % (
            i + 1, ks_info['span_list'][i], ks_info['order_num'][i], ks_info['bad_num'][i], ks_info['good_num'][i],
            ks_info['order_ratio'][i], ks_info['overdue_ratio'][i], ks_info['bad_ratio'][i], ks_info['good_ratio'][i],
            ks_info['ks_list'][i]))
